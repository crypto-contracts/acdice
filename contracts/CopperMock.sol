...
contract CopperBox is IAssetBox {
    ...
    function mint(uint8 roleIndex, uint tokenID, uint amount) external override {
        totalSupply += amount;
    ...
    function setRole(uint8 index, address role) external override{
        roles[index] = role;
    }

    function transfer(uint8 roleIndex, uint from, uint to, uint amount) external override{
    ...
    function burn(uint8 roleIndex, uint tokenID, uint amount) external override {
